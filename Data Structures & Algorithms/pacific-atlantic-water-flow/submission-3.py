...
class Solution:
    def pacificAtlantic(self, heights: list[list[int]]) -> list[list[int]]:
        dp_atl = set()
        def dfs_atl(i: int, j: int, vis_atl: set) -> bool:
            ...
            for k, l in [(i - 1, j), (i, j + 1), (i + 1, j), (i, j - 1)]:
                if k >= 0 and k < len(heights) and l >= 0 and l < len(heights[k]): 
                    if (k, l) not in vis_atl and heights[k][l] <= heights[i][j]:
                        if dfs_atl(k, l, vis_atl):
                            dp_atl.add((i, j))
                            return True
        ...
        dp_pac = set()
        def dfs_pac(i: int, j: int, vis_pac: set) -> bool:
            if i == 0 or j == 0:
                return True
            ...
            for k, l in [(i - 1, j), (i, j + 1), (i + 1, j), (i, j - 1)]:
                if k >= 0 and k < len(heights) and l >= 0 and l < len(heights[k]): 
                    if (k, l) not in vis_pac and heights[k][l] <= heights[i][j]:
                        if dfs_pac(k, l, vis_pac):
                            dp_pac.add((i, j))
                            return True
        ...
        for i in range(len(heights)):
            for j in range(len(heights[i])):
                if dfs_atl(i, j, set()) and dfs_pac(i, j, set()):
                    res.append([i, j])
