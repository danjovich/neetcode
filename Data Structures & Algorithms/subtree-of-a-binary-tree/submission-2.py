...
class Solution:
    ...
    def isSubtree(self, root: Optional[TreeNode], subRoot: Optional[TreeNode]) -> bool:
        # ideal solution using serialization
        # time: O(m + n), space: O(m + n)
        s_root, s_sub_root = self.serialize(root), self.serialize(subRoot)

        return s_sub_root in s_root
    ...
    def isSubtreeDFS(
        ...
        return self.isSubtreeDFS(root.left, subRoot) or self.isSubtreeDFS(
            root.right, subRoot
        )
...
if __name__ == "__main__":
    ...
    def print_and_assert(inp1: TreeNode, inp2: TreeNode, exp: bool):
        res = sol.isSubtreeDFS(inp1, inp2)
        print(res)
        assert res == exp
